...
namespace Framework.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        {
            _busClient.SubscribeAsync<TCommand, CorrelationContext>(async (command, correctionContext) =>
            {
                var commandHandler = _serviceProvider.GetService<ICommandHandler<TCommand>>();

                return await TryHandleAsync(command, correctionContext,
                    () => commandHandler.HandleAsync(command, correctionContext));
            },
